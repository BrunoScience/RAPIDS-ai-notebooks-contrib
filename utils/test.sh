...
# Every repo is submoduled into notebooks have been stored
# into a "notebooks" dir, this loop finds all notebooks specifically added to CI
for nb in $(find /notebooks/ -name *.ipynb); do
    nbBasename=$(basename ${nb})
    # Output of find command looks like this: ./notebooks/<notebook> -name
    ...
    if ((echo ${nb}|grep -qi dask) || \
        ...
        echo "--------------------------------------------------------------------------------"
    elif [[ ${nbRepo} == "conference_notebooks" ]]; then
        echo "--------------------------------------------------------------------------------"
        echo "SKIPPING: ${nb} (Conference Notebooks)"
        echo "--------------------------------------------------------------------------------"
    elif [[ ${nbRepo} == "the_archive" ]]; then
        echo "--------------------------------------------------------------------------------"
        echo "SKIPPING: ${nb} (Archived Notebooks)"
        echo "--------------------------------------------------------------------------------"
    else
